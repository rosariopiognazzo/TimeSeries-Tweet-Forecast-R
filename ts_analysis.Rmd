...
# # Funzione per applicare tsCV sul training set
# cross_validation <- function(series, model_type, h = 1) {
#   if (model_type == "ARIMA") {
#     errors <- tsCV(series, function(y, h) {
...
for (series_name in names(score_list)) {
  ...
  # # Cross-validation sul training set per ARIMA
  # arima_cv_errors <- cross_validation(train, "ARIMA", h = 1)
  # arima_cv_metrics <- calculate_metrics(arima_cv_errors)
  # arima_cv_metrics <- mutate(arima_cv_metrics, Serie = series_name, Model = "ARIMA", Stage = "Cross-Validation")
  ...
  # # Cross-validation sul training set per NNETAR
  # nnetar_cv_errors <- cross_validation(train, "NNETAR", h = 1)
  # nnetar_cv_metrics <- calculate_metrics(nnetar_cv_errors)
  # nnetar_cv_metrics <- mutate(nnetar_cv_metrics, Serie = series_name, Model = "NNETAR", Stage = "Cross-Validation")
